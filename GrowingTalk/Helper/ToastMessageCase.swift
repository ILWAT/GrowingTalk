...
enum ToastMessageCase{
    enum Signup: String {
        ...
        case etc = "에러가 발생했어요. 잠시후 다시 시도해주세요."
    }
    
    enum WorkSpaceAdd: String {
        case nameIsNotValid = "워크스페이스 이름은 1~30자로 설정해주세요."
        case imageRequired = "워크스페이스 이미지를 등록해주세요. "
        case etc = "알 수 없는 에러가 발생했어요. 잠시후 다시 시도해주세요."
    }
}
